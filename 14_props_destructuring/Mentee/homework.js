...
let products = [
    {title: "Game", price: 59, stock: 6 },
    {title: "Controller", price: 129, stock: 3 },
    {title: "Headset", price: 212, stock: 0 }
 ];
...
function createProductCard ({ title, price, stock }){
    const priceText = "$" + price.toFixed(2);
    const stockMessage = stock === 0 ? "Out of stock" : `In stock: ${stock} items`;
    const stockClass = stock === 0 ? "out" : "in";
...
generateProductsBtn.addEventListener("click", () => {
    ...
    products.forEach((product) => {
        cardsHTML = cardsHTML + createProductCard(product)
    });
    productContainer.innerHTML= cardsHTML;
} );